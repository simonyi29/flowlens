...
const userGroups = [
  { labelKey: 'groups.workspace', items: [
  ...
  { labelKey: 'groups.data', items: [
    { to: '/library', labelKey: 'nav.library', icon: Library },
    { to: '/media', labelKey: 'nav.media', icon: Clapperboard },
  ] },
  { labelKey: 'groups.automation', items: [{ to: '/schedules', labelKey: 'nav.schedules', icon: CalendarClock }] },
  { labelKey: 'groups.account', items: [
...
function Brand() {
  ...
  return <Link to="/" className="flex min-h-16 items-center gap-3 border-b border-slate-200 px-5"><img src="/logos/flowlens-favicon.png" width="32" height="32" alt="" className="h-8 w-8 rounded-lg"/><div><div className="text-sm font-semibold tracking-tight text-slate-950">FlowLens</div><div className="text-[11px] text-slate-500">{t('brandSubtitle')}</div></div></Link>
}

function UserSidebar({ open, close }: { open: boolean; close: () => void }) {
  const { t } = useTranslation('product')
  return <><button aria-label={t('shell.closeOverlay')} className={cn('fixed inset-0 z-40 bg-slate-950/30 backdrop-blur-[1px] lg:hidden', open ? 'block' : 'hidden')} onClick={close}/><aside className={cn('fixed inset-y-0 left-0 z-50 flex w-[264px] flex-col border-r border-slate-200 bg-white transition-transform lg:sticky lg:top-0 lg:h-screen lg:shrink-0 lg:translate-x-0', open ? 'translate-x-0' : '-translate-x-full')}>
    <div className="relative"><Brand/><button onClick={close} className="absolute right-3 top-3 grid h-10 w-10 place-items-center rounded-lg text-slate-500 hover:bg-slate-100 lg:hidden" aria-label={t('shell.closeNavigation')}><X className="h-5 w-5"/></button></div>
    <nav className="flex-1 space-y-5 overflow-y-auto px-3 py-5">{userGroups.map(group => <div key={group.labelKey}><p className="mb-1 px-3 text-[11px] font-semibold uppercase tracking-wider text-slate-400">{t(group.labelKey)}</p><div className="space-y-1">{group.items.map(item => <NavLink key={item.to} to={item.to} end={item.end} onClick={close} className={({ isActive }) => cn('flex min-h-10 items-center gap-3 rounded-lg px-3 text-sm font-medium transition-colors', isActive ? 'bg-teal-50 text-teal-800' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-950')}><item.icon className="h-[18px] w-[18px]"/><span>{t(item.labelKey)}</span></NavLink>)}</div></div>)}</nav>
...
function TopBar({ openMenu, onShowDisclaimer, capabilities }: { openMenu: () => void; onShowDisclaimer: () => void; capabilities: Capabilities | null }) {
  ...
  return <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-slate-200 bg-white/95 px-2 backdrop-blur sm:px-6 lg:px-8"><div className="flex min-w-0 items-center gap-2 sm:gap-3"><button onClick={openMenu} className="grid h-11 w-11 shrink-0 place-items-center rounded-lg text-slate-600 hover:bg-slate-100 lg:hidden" aria-label={t('shell.openNavigation')}><Menu className="h-5 w-5"/></button><div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-900">{title}</p><p className="hidden text-xs text-slate-500 sm:block">{t('shell.subtitle')}</p></div></div><div className="flex shrink-0 items-center gap-1 sm:gap-2"><button onClick={onShowDisclaimer} className="hidden min-h-8 items-center rounded-full bg-amber-50 px-3 text-xs font-medium text-amber-800 ring-1 ring-inset ring-amber-200 sm:inline-flex">{t('shell.researchOnly')}</button>{capabilities?.features.admin ? <Link to="/admin/workers" className="hidden min-h-9 items-center gap-2 rounded-lg px-3 text-xs font-medium text-slate-600 hover:bg-slate-100 md:flex"><ShieldCheck className="h-4 w-4"/>{t('shell.admin')}</Link> : null}<ThemeToggle/><LanguageSwitch/></div></header>
}
...
function MobileNavigation() {
  ...
  return <nav className="fixed inset-x-0 bottom-0 z-30 grid h-16 grid-cols-4 border-t border-slate-200 bg-white/95 px-2 pb-[env(safe-area-inset-bottom)] backdrop-blur lg:hidden">{items.map(item => <NavLink key={item.to} to={item.to} end={item.end} className={({ isActive }) => cn('flex flex-col items-center justify-center gap-1 text-[11px] font-medium', isActive ? 'text-teal-700' : 'text-slate-500')}><item.icon className="h-5 w-5"/><span>{t(item.labelKey)}</span></NavLink>)}</nav>
}
...
export function UserAppShell({ onShowDisclaimer }: { onShowDisclaimer: () => void }) {
  ...
  return <div className="flex min-h-screen items-start bg-app-canvas text-slate-900"><a href="#main-content" className="fixed left-3 top-3 z-[100] -translate-y-20 rounded-lg bg-slate-950 px-4 py-2 text-sm text-white focus:translate-y-0">{t('skipToContent')}</a><UserSidebar open={menuOpen} close={() => setMenuOpen(false)}/><div className="flex min-h-screen min-w-0 flex-1 flex-col"><TopBar openMenu={() => setMenuOpen(true)} onShowDisclaimer={onShowDisclaimer} capabilities={capabilities}/><main id="main-content" className="mx-auto w-full max-w-[1560px] flex-1 px-4 pb-24 pt-6 sm:px-6 lg:px-8 lg:pb-8"><Outlet context={{ capabilities }}/></main><footer className="hidden border-t border-slate-200 bg-white px-8 py-3 text-xs text-slate-500 lg:flex lg:items-center lg:justify-between"><span>{t('shell.footer')}</span><a href="https://github.com/simonyi29/flowlens" target="_blank" rel="noreferrer" className="hover:text-teal-700">{t('shell.repository')}</a></footer></div><MobileNavigation/></div>
}
