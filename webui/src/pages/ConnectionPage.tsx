...
import { AlertTriangle, CheckCircle2, Clock3, Link2, Loader2, Monitor, RefreshCw, ShieldCheck, Smartphone, Unlink } from 'lucide-react'
...
function RemoteConnection() {
  ...
  const start = async () => {
    if (!worker) return
    setBusy(true)
    try { const response = await remoteApi.createLogin(String(worker.worker_id)); setQrUrl(''); setSession(response.data) }
    catch { toast.error('无法创建登录会话，请确认执行设备在线') } finally { setBusy(false) }
  }
  const refresh = async () => {
    if (!session) return
  ...
  const reconnect = async () => {
    ...
    const response = await remoteApi.reconnect(String(activeConnection.connection_id)); setQrUrl(''); setSession(response.data)
  }
  const disconnect = async () => {
    if (!activeConnection || !confirm('断开后会删除执行设备上的专用浏览器会话，历史采集结果不会删除。确认继续？')) return
    await remoteApi.disconnect(String(activeConnection.connection_id)); toast.success('账号已断开'); await load()
  }
  ...
  return <div><PageHeader eyebrow="账号" title="抖音账号" description="扫码登录发生在执行设备的独立浏览器中。Cookie、浏览器会话和技术参数不会上传到网站。"/>
    {activeConnection && !liveSession ? <Surface className="overflow-hidden"><div className="p-6 sm:p-8"><div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between"><div className="flex gap-4"><div className="grid h-12 w-12 place-items-center rounded-xl bg-teal-50 text-teal-700"><CheckCircle2 className="h-6 w-6"/></div><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-lg font-semibold text-slate-950">{String(activeConnection.masked_nickname || '已连接账号')}</h2><StatusBadge status={String(activeConnection.status)}/></div><p className="mt-2 text-sm text-slate-600">此账号可以用于创建抖音采集任务。</p></div></div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={load}><RefreshCw/>检查登录</Button>{['session_expired','verification_required','risk_controlled'].includes(String(activeConnection.status)) ? <Button onClick={reconnect}>重新扫码</Button> : null}<Button variant="destructive" onClick={disconnect}><Unlink/>断开账号</Button></div></div></div><div className="grid border-t border-slate-200 sm:grid-cols-3"><Info label="执行设备" value={String(worker?.name || '未分配')}/><Info label="最后验证" value={formatDate(String(activeConnection.last_verified_at || ''))}/><Info label="账号标识" value={String(activeConnection.creator_hash || '登录后生成')}/></div></Surface> : liveSession ? <Surface className="mx-auto max-w-3xl overflow-hidden"><div className="grid md:grid-cols-[320px_1fr]"><div className="grid min-h-80 place-items-center bg-slate-50 p-8"><div className="relative aspect-square w-full max-w-[240px] rounded-2xl bg-white p-3 shadow-sm ring-1 ring-slate-200">{qrUrl ? <img src={qrUrl} alt="抖音登录二维码" className="h-full w-full object-contain"/> : <div className="grid h-full place-items-center"><Loader2 className="h-8 w-8 animate-spin text-teal-700"/></div>}</div></div><div className="flex flex-col justify-center p-6 sm:p-8"><div className="flex items-center gap-2 text-xs font-medium text-slate-500"><Clock3 className="h-4 w-4"/>二维码有效期约 {expiresAt || 180} 秒</div><h2 className="mt-4 text-xl font-semibold text-slate-950">{loginText[String(session.status)] || String(session.status)}</h2><p className="mt-2 text-sm leading-6 text-slate-600">打开抖音 App 扫描左侧二维码，并按手机提示确认。登录成功后二维码会立即失效。</p>{['captcha_required','risk_controlled'].includes(String(session.status)) ? <div className="mt-5 flex gap-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0"/><span>需要管理员在执行设备处理验证，系统不会绕过验证码。</span></div> : null}<div className="mt-6 flex gap-2"><Button variant="outline" onClick={refresh}><RefreshCw/>刷新二维码</Button><Button variant="ghost" onClick={() => remoteApi.cancelLogin(String(session.login_session_id)).then(() => { setSession(null); setQrUrl('') })}>取消</Button></div></div></div></Surface> : <Surface><EmptyState icon={worker ? <Smartphone/> : <AlertTriangle/>} title={worker ? '尚未连接抖音账号' : '执行设备离线'} description={worker ? '连接后可使用该账号执行公开数据采集。Cookie 和浏览器会话只保留在抓取设备。' : '当前没有可用的执行设备，请联系管理员检查 Worker 状态。'} action={<Button size="lg" disabled={!worker || busy} onClick={start}>{busy ? <Loader2 className="animate-spin"/> : <Link2/>}连接抖音账号</Button>}/></Surface>}
